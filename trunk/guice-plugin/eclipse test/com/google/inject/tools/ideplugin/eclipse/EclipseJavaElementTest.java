...
public class EclipseJavaElementTest extends TestCase {
  ...
  private void setUpVariable() {
    ...
    EasyMock.expect(variable.getTypeSignature()).andReturn(
        "Lcom.google.inject.tools.ideplugin.sample.eclipse.TestVariableClass;")
        .anyTimes();
    EasyMock.expect(variable.getElementName()).andReturn("TestVariable")
        .anyTimes();
    EasyMock.expect(variable.getJavaProject()).andReturn((IJavaProject) null)
        .anyTimes();
    EasyMock.replay(variable);
    IType type = EasyMock.createMock(IType.class);
    EasyMock.expect(type.getFullyQualifiedName()).andReturn(
        "com.google.inject.tools.ideplugin.sample.eclipse.TestVariableClass");
  ...
  public void testElementFields() {
    setUpVariable();
    assertTrue(element.getClassName().equals(
        "com.google.inject.tools.ideplugin.sample.eclipse.TestVariableClass"));
