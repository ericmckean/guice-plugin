...
/**
 * The CodeRunner runs {@link CodeSnippet} objects in a separate virtual machine that also
 * has access to the user's class files.
 * 
...
public interface CodeRunner {
  ...
  /**
   * An exception thrown if the CodeRunner is asked to run a non {@link CodeSnippet} object.
   */
  ...
  /**
   * An exception thrown if the CodeRunner received notification of a result that is not the
   * result of a {@link CodeSnippet}.
   */
   ...
   * 
   * It must implement the static main(String[] args) method and create a result that extends
   * {@link CodeSnippetResult} which should be written to System.out as an object.
   */
  public static abstract class Runnable {
    ...
    /**
     * Return the arguments to pass to the code snippet we are running.
     * NOTE: This is called by the constructor so must not access any fields.
     */
    protected abstract List<? extends Object> getSnippetArguments();
    
    /**
     * Return the (fully qualified) name of the {@link CodeSnippet} class to run.
     */
